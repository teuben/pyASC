...
WEBDIR =   	/Users/eric/Sites/js9
CGIDIR =   	
CGIURL =   	
CGIXPATH =   	
# this is for SAO install only ... I think!
SAODIR =	/Users/eric/Sites
...
saoprefs:	FORCE
		@(VER=`node --version | tr -d v | awk -F. '{print $$1 * 100 + $$2}'`; \
		if [ $$VER -lt 1016 ]; then \
		    echo "editing js9Prefs for SAO installations (v2) ..."; \
		    sed -f $(BSCR)/saoprefs.sed < js9Prefs.json > sao/js9Prefs.json;\
		    sed -f  $(BSCR)/saoprefs.sed < js9prefs.js  > sao/js9prefs.js;\
		else \
		    echo "editing js9Prefs for SAO installations (v3) ..."; \
		    sed -f $(BSCR)/saoprefs-v3.sed < js9Prefs.json > sao/js9Prefs.json;\
		    sed -f  $(BSCR)/saoprefs-v3.sed < js9prefs.js  > sao/js9prefs.js;\
		fi; \
...
install-webdirs:	FORCE
	@(if [ x$(WEBDIR) != x ]; then \
	 ...
	 if [ x$(USEHELPER) = xyes ]; then \
	   if [ -d $(WEBDIR)/node_modules/socket.io-3 ]; then \
	     echo "removing backend dir: node_modules"; \
